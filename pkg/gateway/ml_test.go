...
func TestSyncMLProject_Success(t *testing.T) {
	...
	err := client.SyncMLProject(context.Background(), MLProjectItem{
...
func TestSyncMLRun_OneRequestPerItem(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(w).Encode(mlSyncResponse{Synced: len(items)})
	}))
	defer server.Close()
	...
	for _, run := range runs {
		if err := client.SyncMLRun(context.Background(), run); err != nil {
...
func TestSyncMLProject_ErrorStatus(t *testing.T) {
	...
	err := client.SyncMLProject(context.Background(), MLProjectItem{Name: "P", Type: "model"})
	if err == nil {
